...
async function main() {
    ...
    for (const material of allMaterials) {
        const stockCount = Math.floor(Math.random() * 3) + 2;
        for (let i = 0; i < stockCount; i++) {
            ...
            const quantity = isLowStock
                ? Math.floor(minStock * 0.5)
                : Math.floor(minStock * (2 + Math.random() * 3));
            const receivedDate = new Date();
            receivedDate.setDate(receivedDate.getDate() - Math.floor(Math.random() * 60));
            let expiryDate = null;
    ...
    for (const recipeData of recipesData) {
        ...
        const paramCount = Math.floor(Math.random() * 3) + 4;
    ...
    for (let i = 0; i < 12; i++) {
        ...
        plannedDate.setDate(plannedDate.getDate() - Math.floor(Math.random() * 90));
        const deadline = new Date(plannedDate);
        deadline.setDate(deadline.getDate() + Math.floor(Math.random() * 14) + 7);
        const isOverdue = Math.random() < 0.2 && status === 'IN_PROGRESS';
        if (isOverdue) {
            deadline.setDate(deadline.getDate() - Math.floor(Math.random() * 10) - 5);
        }
        await prisma.productionOrder.create({
            data: {
                orderNumber: `ORD-2026-${String(i + 1).padStart(3, '0')}`,
                recipeId: recipe.id,
                plannedQty: Math.floor(Math.random() * 400) + 100,
                unit: 'kg',
                status,
                plannedDate,
                deadline,
    ...
    for (let i = 0; i < 20; i++) {
        ...
        const producedQty = Math.floor(Math.random() * 150) + 50;
        ...
        if ((status === 'COMPLETED' || status === 'DEFECTIVE') && recipe) {
            ...
            for (const ingredient of recipeIngredients) {
                const usedQty = Number(ingredient.quantity) * producedQty / 100;
    ...
    for (let i = 0; i < 5; i++) {
        ...
        for (const material of selectedMaterials) {
            const systemQty = Math.floor(Math.random() * 100) + 50;
    ...
    for (let i = 0; i < 15; i++) {
        ...
        plannedStart.setDate(plannedStart.getDate() - Math.floor(Math.random() * 180) + 30);
        const plannedEnd = new Date(plannedStart);
        plannedEnd.setDate(plannedEnd.getDate() + Math.floor(Math.random() * 7) + 3);
        ...
        const engineerCount = Math.floor(Math.random() * 3) + 1;
        ...
        if (['IN_PROGRESS', 'COMPLETED', 'ON_HOLD'].includes(status) && actualStart) {
            ...
            for (let j = 0; j < logCount; j++) {
                ...
                endTime.setHours(startTime.getHours() + Math.floor(Math.random() * 6) + 3);
        ...
        if (Math.random() < 0.4 && ['IN_PROGRESS', 'COMPLETED'].includes(status)) {
            ...
            const usedPartsCount = Math.floor(Math.random() * 3) + 1;
